% this script is written to be run for a folder of .mat files - it was
% modified from Wei's FI analysis script to analyze a series of
...
save_file_name = 'm1_3sh_CCh_Analysis.mat';
...
% voltage clamp seal test
sealtestend  = 1050; % in ms
sealteststart  = 0.5; % in s
sealtestlength  = 0.5; % in s
sealtest_dvolt = 0.005; % V

% current clamp seal test (negative pulse)
sealtest_dcurr = 100e-12; % A (100 pA)
iteststart = 0.5; % in s
itestlength = 0.5; % in s

% current clamp depolarizing pulse
pulse_dcurr = 200e-12; % A (200 pA)
samplerate = 10000;
%%
close all
for jj = 1:numel(experiment)
    ...
    trace_num = cell(1,file_num);
    trace_ID = cell(1, file_num);
    ...
    Rin_IC = cell(1, file_num);
    Cp_IC = cell(1, file_num);
    ...
    for f = 1:file_num
       ...
       data = data.sweep0;
            ...
            if ~strcmp(cellName, curr_cell)
                ...
                NUM = A{f}(end-6:end-4);
                if NUM(1) == '_'
                    trace_id = str2double(NUM(2:3));
                else
                    trace_id = str2double(NUM);
                end
            end
            
            
        % also make sure to save trace number somewhere!! Need to be able
        % to cross ref with notes and figure out timing - each trace is 5
        % sec
        
        
        % if data y value at 0.2 seconds > -40; is VC trace! run
        % get_PassProp_VC
        if mean(data(2000:3000)) > -40
             % pull passive properties!
            [Ra, Cp, Rin, Vr] = get_PassProp_VC(data .* 10^-12, sealteststart, sealtestlength, sealtest_dvolt, samplerate);
            all_Ra{1,cellID}(trace_id, 1) = Ra;
            all_Rin{1,cellID}(trace_id, 1) = Rin;
            all_Cp{1,cellID}(trace_id, 1) = Cp;
            all_Vr{1,cellID}(trace_id, 1) = Vr;
                    
        %elseif data y value at 0.7 seconds > data y value at 0.2 seconds;
        %is CC depolarizing pulse - run FI code
        elseif mean(data(7000:8000)) > mean(data(2000:3000))
  
            % IS depolarizing current TRACE

            trace_num{1,cellID}(trace_id,1) = trace_id; % keep track of which traces are current depolarizations to analyze spiking data
            aDAT{1,cellID}(1:length(data),trace_id) = data;
            
        % elseif data y value at 0.7 seconds < data y value at 0.2 seconds;
        % is CC hyperpolarizing pulse - run get_PassProp_IC
        elseif mean(data(7000:8000)) < mean(data(2000:3000))
            
            [Cp_est, Rin_CC] = get_PassProp_IC(data .* 10^-3, iteststart, itestlength, sealtest_dcurr, samplerate);
            Rin_IC{1,cellID}(trace_id,1) = Rin_CC;
            Cp_IC{1,cellID}(trace_id,1) = Cp_est;
    ...
    for gi = 1:cellID
        trace_num{1,gi} = nonzeros(trace_num{1,gi});
        if isempty(trace_num{1,gi}) == 1
            continue
        else
            for ri = 1:trace_num{1,gi}(end,1)
                if ismember(ri,trace_num{1,gi}) == 0
                    aDAT{1,gi}(:,ri) = NaN;
                end
            end
        end
    ...
    for ci = 1:cellID
        ...
        if isempty(trace_num{1,ci}) == 1
            continue
        else

            for ti = 1:trace_num{1,ci}(end,1)
                if ismember(ti,trace_num{1,ci}) == 0
                    MFR{1,ci}(ti,1) = NaN;
                    ISI{1,ci}(ti) = NaN;
                    IFR{1,ci}(ti) = NaN; 
                    V_th{1,ci}{ti} = {};
                    ...
                    AHP{1,ci}(ti,1) = min(data(11000:15000)) - nanmean(data(1000:2000));
                    
                    % Measure ADP (re: Gulledge et al. 2009)
                    ADP{1,ci}(ti,1) = max(data(11000:25000)) - nanmean(data(1000:2000));

                    %Detect AP in each trace
                    sp_start = iteststart;
                    sp_end = iteststart + itestlength;
                    [dV_sec_temp, V_th_temp, f_trough_temp, sp_num_temp, udratio_temp] = get_vthresh_flex(data, sp_start, sp_end, plot_on);
...
clear FI_DATA
    ...
    FI_DATA.trace_num = trace_num(1:cellID);
    ...
    FI_DATA.Rin_IC = Rin_IC(1:cellID);
    FI_DATA.Cp_IC = Cp_IC(1:cellID);
...
%     display_name = strcat(current_dir_name(total_dir_name_number-5: end),'.mat');
    save(save_file_name,'FI_DATA', '-v7.3')
end